...
if __name__ == "__main__":
    ...
    for i in range(episodes):
        ...
        while not game.is_episode_finished():
            ...
            max = np.max(depthmap)
            direction = np.where(depthmap == np.max(depthmap))[0]
            if max > 100:
                if direction[0] * 2 > depthmap.shape[1]:
                    action = [False, False, False, False, True, False]
                elif direction[0] * 2 < depthmap.shape[1]:
                    action = [False, False, False, False, False, True]

                current = node

                # while get_distance_to_next(node[0], node[1], current[0], current[1]) < 30:
                if get_distance_to_next(node[0], node[1], current[0], current[1]) < 100:
                    action = [False, False, True, False, False, False, False]
